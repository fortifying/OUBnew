...
async def deploy(event, repo, ups_rem, ac_br, txt):
    if HEROKU_API_KEY is not None:
        ...
        try:
            remote.push(refspec="HEAD:refs/heads/phoenix", force=True)
...
async def upstream(event):
    ...
    try:
        ...
        origin.fetch()
        force_update = True
        repo.create_head('phoenix', origin.refs.phoenix)
        repo.heads.phoenix.set_tracking_branch(origin.refs.phoenix)
        repo.heads.phoenix.checkout(True)
